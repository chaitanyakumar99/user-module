...
const generateOtp = async (req, res) => {
  try {
    ...
    //check weather phone number exist or not
    if (!phoneNumber) {
      res.status(400).json({
        status: "Failed",
    ...
    //store the hashed otp and otp expiry time
...
const verifyOtp = async (req, res) => {
  try {
    ...
    //compare otp weather it's valid or not
    ...
    //remove the otp token and exiry from user
